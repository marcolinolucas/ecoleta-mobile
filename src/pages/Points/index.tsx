...
import { getItems, getPoints } from '../../services/EcoletaService';
...
const Points = () => {
	...
	function handleNavigateToDetail({ id }: { id: number }) {
		navigation.navigate('Detail', { pointId: id });
	}

	function handleSelectItem({ id }: {id: number }) {
		...
		<>
			<View style={styles.container}>
				...
				<View style={{flexDirection: 'row'}}>
					<Emoji name="grinning" style={{paddingTop: 20, paddingRight: 10, fontSize: 20 }} />
						...
						>
							{points.map(point => (
								<Marker
									...
									onPress={() => handleNavigateToDetail({ id: point.id })}
									coordinate={{
				...
				>
					{items.map(item => (
						<TouchableOpacity
							...
							activeOpacity={0.6}
							onPress={() => handleSelectItem({ id: item.id })}>
